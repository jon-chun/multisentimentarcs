...
def process_file(file_path: str, output_dir: str):
    ...
    print("Initial VADER Sentiment Values:\n", df['sentiment-vader'].head())
    print("Initial TextBlob Sentiment Values:\n", df['sentiment-textblob'].head())
...
def timeseries_norm(df, df_col_list, sma_per):
    try:
        ...
        for col in df_col_list:
            ...
            print(f"SMA Smoothed {col} Values:\n", smoothed_col.head())

            # Z-score Normalization
            normalized_col = zscore(smoothed_col)
            print(f"Z-score Normalized {col} Values:\n", normalized_col[:5])
...
def plot_kde(df, df_col_list, output_dir="plots", plot_title="KDE Distributions", file_name="kde_distributions.png"):
    """
    ...
    output_plot_path = os.path.join(output_dir, file_name)
    ...
    for file_name in os.listdir(input_dir):
        if file_name.endswith('.json'):
            ...
            if df is not None:
                ...
                plot_kde(df, ['sentiment-vader_norm','sentiment-textblob_norm'], output_dir="plots", plot_title="Sample KDE Distributions", file_name="sample_kde_distributions.png")
            ...
            # Apply the KDE plot function
            plot_kdes(df, ['value1', 'value2'], output_dir="plots", plot_title="Sample KDE Distributions", file_name="sample_kde_distributions.png")
