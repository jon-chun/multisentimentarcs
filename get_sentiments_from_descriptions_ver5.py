...
def process_file(subdir_path: str, output_dir: str, save_interval=SAVE_INTERVAL):
    try:
        text_list = read_text_files(subdir_path)
        ...
        output_file_name = f"{os.path.basename(subdir_path)}{output_extension}"
        output_file_path = os.path.join(output_dir, output_file_name)
        for i in range(0, len(df), save_interval):
            df.iloc[:i + save_interval].to_csv(output_file_path, index=False)
        return df
    except Exception as e:
        logging.error(f"Error processing directory {subdir_path}: {e}")
        return None
...
def plot_sentiment_sma(df, df_cols_list=['sentiment-vader_norm', 'sentiment-textblob_norm', 'sentiment-ollama_norm'], win_per=10, film_name='', film_year='', output_dir=''):
    try:
        ...
        plt.legend()
        plt.grid(True)
        output_plot_path = os.path.join(output_dir, f'plot_{film_name}_{film_year}_sentiment.png')
...
def main():
    for subdirs_names in os.listdir(input_dir):
        ...
        if os.path.isdir(subdir_path):
            output_subdir_path = os.path.join(output_dir, subdirs_names)
            if not os.path.exists(output_subdir_path):
                os.makedirs(output_subdir_path)

            output_file_name = f"{subdirs_names}{output_extension}"
            ...
            if os.path.exists(output_file_path):
                if OVERWRITE_SRT_FLAG:
                    logging.info(f"Output file exists. Overwriting file: {output_file_name}")
                    df = process_file(subdir_path, output_subdir_path)
                    if df is not None:
                        df.to_csv(output_file_path, index=False)
                else:
                    logging.info(f"Output file already exists. Skipping directory: {subdirs_names}")
                    continue
            else:
                df = process_file(subdir_path, output_subdir_path)
                if df is not None:
                    df.to_csv(output_file_path, index=False)
            if df is not None:
                film_name = "_".join(subdirs_names.split('_')[:-1])
                film_year = subdirs_names.split('_')[-1]
                logging.info("Plotting sentiment SMA...")
                plot_sentiment_sma(df, df_cols_list=['sentiment-vader_norm', 'sentiment-textblob_norm', 'sentiment-ollama_norm'], win_per=10, film_name=film_name, film_year=film_year, output_dir=output_subdir_path)
                logging.info("Sentiment SMA plotted and saved.")
                logging.info("Plotting KDE distributions...")
                plot_kde(df, ['sentiment-vader_norm', 'sentiment-textblob_norm', 'sentiment-ollama_norm'], film_name=film_name, film_year=film_year, output_dir=output_subdir_path, plot_title="KDE Distributions", file_suffix=f"_{file_suffix}_sentiments.png")
