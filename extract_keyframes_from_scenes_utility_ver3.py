...
def extract_keyframes(base_output_path):
    if TARGET_FILMS_LIST:
        ...
        for target_film in TARGET_FILMS_LIST:
            ...
            scene_file_path = os.path.join(base_output_path, film_directory, scene_file)
            if os.path.exists(scene_file_path):
                process_scene_file(os.path.join(base_output_path, film_directory), scene_file)
            else:
    ...
    with open(scene_file_path, 'r') as csvfile:
        ...
        for row in reader:
            try:
                ...
                frame_filename = f"scene{scene_no}_{film_base_name}.png"
