...
TARGET_FILM_LIST = [
    os.path.join("..","data","videos","adventure","Captain_Kidd_1945.mp4"),
    os.path.join("..","data","videos","comedy","Gentlemen_Prefer_Blondes_1953.mp4"),
    os.path.join("..","data","videos","filmnoir","Notorious_1946.mp4"),
    os.path.join("..","data","videos","western","Rawhide_1951.mp4")
...
def sanitize_filename(filename):
    ...
    sanitized_name_final = sanitized_name.replace(' ', '_')
    logging.info(f"Sanitized filename final: {sanitized_name_final}")
    return sanitized_name_final
...
def process_video_file(input_file_path, output_subdir):
    ...
    for attempt in range(MAX_RETRY_ATTEMPTS):
        ...
        print(f' PARTIAL SCENE COUNT: {partial_scene_count}')
        print(f' EXTRAPOLATED SCENE COUNT: {extrapolated_scene_count}')
        print(f' COMPARING PARTIAL_MIN_SCENE_COUNT: {PARTIAL_MIN_SCENE_COUNT}')
        print(f' COMPARING PARTIAL_MAX_SCENE_COUNT: {PARTIAL_MAX_SCENE_COUNT}')
...
def main():
    ...
    if len(TARGET_FILM_LIST) == 0:
        # Crawl over subdirectories under root
        try:
            ...
            try:
                for filename in filename_list:
                    if filename.endswith(".mp4"):
                        ...
                        output_subdir = create_output_subdir(output_dir_base_path, input_file_path)
        ...
        logging.info(f"Processing films in TARGET_FILM_LIST: {TARGET_FILM_LIST}")
        filename_list = sorted(TARGET_FILM_LIST)

        for filename_path in filename_list:
            logging.info(f"\n\nPROCESSING (full path to *.mp4): {filename_path}")

            filename_only = os.path.basename(filename_path)
            filename_clean = sanitize_filename(filename_only)
            directory_path = os.path.dirname(filename_path)
            filename_genre = os.path.basename(directory_path)
            filepath_base = os.path.dirname(directory_path)

            try:
                if filename_path.endswith(".mp4"):
                    input_file_path = filename_path
                    logging.info(f"Found MP4 file: {input_file_path}")

                    previous_runs = history_df[history_df["filename"] == filename_clean]
                    logging.info(f"Previous runs for {filename_clean}:\n{previous_runs}")

                    output_subdir = create_output_subdir(filepath_base, filename_genre, filename_only) # input_file_path)
                    process_video_file(input_file_path, output_subdir)
                ...
                logging.error(f"Error processing filepath {filename_path}: {e}")
                continue
