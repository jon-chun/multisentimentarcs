    ...
    for attempt in range(1, MAX_CALL_OLLAMA + 1):
        try:
            ...
            if 'message' in res and 'content' in res['message']:
                ...
                print(f'    TEXT_SENTIMENT_FLOAT_STR = [{text_sentiment_float_str}]')
                try:
...
def process_file(file_path: str, output_dir: str, save_interval=SAVE_INTERVAL):
    # Read the JSON file
    print(f" in process_file(file_path={file_path}), output_dir={output_dir}")

    text_list = read_text_files(file_path)
...
def timeseries_norm(df, df_col_list=['sentiment-vader_norm', 'sentiment-textblob_norm', 'sentiment-ollama_norm'], sma_per=10):
    try:
...
def main():
    ...
    for subdirs_names in os.listdir(input_dir):
        print(f"  subdirs_names: {subdirs_names}")
        stills_list = sorted(os.listdir(os.path.join(input_dir,subdirs_names)))
        for stills_name_now in stills_list:
            print(f"  subdirs_names: {subdirs_names}")
            print(f"  stills_name_now: {stills_name_now}")
            print(f"  output_dir: {output_dir}")
            output_dir_path = os.path.join(output_dir, subdirs_names)
            print(f"  output_dir_path: {output_dir_path}")

            if stills_name_now.endswith('.txt'):
                file_path = os.path.join(input_dir, stills_name_now)
                
                # Check if output file already exists
                base_name = os.path.basename(file_path)
                output_file_name = os.path.splitext(base_name)[0] + "_pic_sentiments.csv"
                output_file_trimmed = "_".join(output_file_name.split("_")[1:])
                output_file_path = os.path.join(output_dir, output_file_trimmed)
                ...
                if os.path.exists(output_file_path):         
                    ...
                    # if not OVERWRITE_SRT_FLAG:
                    #     continue
                
                # populate df depending on PLOT_ONLY (TRUE: read in, FALSE: calc & save)
                if PLOT_ONLY:
                    if os.path.exists(output_file_path):
                        df = pd.read_csv(output_file_path)
                        logging.info(f"Loaded existing file: {output_file_path}")
                    else:
                        logging.error(f"File not found: {output_file_path}")
                        continue
                else:
                    # df = process_file(output_file_path, output_dir)
                    file_fullpath = os.path.join(input_dir, subdirs_names, stills_name_now)
                    input_subdir_path = os.path.join(input_dir, subdirs_names)
                    print(f"calling process_file(input_path={file_path}, output_file={output_dir_path})")
                    df = process_file(input_subdir_path, output_dir_path)
            ...
            plot_kde(df, ['sentiment-vader_norm', 'sentiment-textblob_norm', 'sentiment-ollama_norm'], film_name=film_name, film_year=film_year, output_dir=output_dir, plot_title="KDE Distributions", file_suffix="_{file_suffix}_sentiments.png")
            logging.info("KDE distributions plotted and saved.")
        else:
            logging.error(f"Failed to process file: {output_file_path}")
