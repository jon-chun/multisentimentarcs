...
def create_output_subdir(base_output_path, leaf_dir, video_filename):
    ...
    while os.path.exists(output_subdir):
        output_subdir = os.path.join(base_output_path, genre, f"{subdir_name}_{counter}")
        counter += 1
    try:
        os.makedirs(output_subdir, exist_ok=True)
        logging.info(f"Created output subdir: {output_subdir}")
...
def process_video_file(input_file_path, output_subdir):
    ...
    # Check if the output file already exists and is not empty, and skip if OVERWRITE_CSV is False
    if not OVERWRITE_CSV:
        print(f"WE SHOULD SKIP with scenes_file_path: {scenes_file_path} and getsize {os.path.getsize(scenes_file_path)}")
        if os.path.isfile(scenes_file_path):
            if os.path.getsize(scenes_file_path) > MIN_SCENE_FILE_KB * 1024:
                logging.info(f"Output file for {video_filename} already exists and is not empty. Skipping as OVERWRITE_CSV is set to False.")
                return
...
def process_video_file(input_file_path, output_subdir):
    ...
    # Check if the file should be skipped based on history
    if should_skip_file(video_filename):
        logging.info(f"File {video_filename} already processed successfully in the past. Skipping.")
        return
...
def process_video_file(input_file_path, output_subdir):
    ...
    for attempt in range(MAX_RETRY_ATTEMPTS):
        ...
        if PARTIAL_MIN_SCENE_COUNT <= partial_scene_count <= PARTIAL_MAX_SCENE_COUNT:
            ...
            full_scene_detection(input_file_path, output_subdir, current_threshold, video_basename, scenes_file_path)
            # Delete the temporary partial video file
            if os.path.exists(os.path.join(output_subdir, f"{video_filename}_partial.mp4")):
                os.remove(os.path.join(output_subdir, f"{video_filename}_partial.mp4"))
            return
        else:
            # Adjust threshold and retry if the partial scene count is out of range
