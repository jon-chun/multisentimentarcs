...
def timeseries_norm(df, df_col_list=['vader', 'textblob', 'llama3'], sma_per=10):
    try:
        ...
        for col in df_col_list:
            ...
            # Compute SMA on normalized data
            smoothed_col = pd.Series(normalized_col).rolling(window=window_size, min_periods=1).mean()
...
def plot_kde(df=None, df_col_list=['vader_norm', 'textblob', 'llama3_norm'], film_name='', film_year='', output_dir="plots", plot_title="KDE Distributions", file_suffix="_kde_plot.png"):
    try:
