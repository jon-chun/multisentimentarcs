...
def save_description(leaf_dir: str, keyframe: str, description: str) -> None:
    ...
    output_dir = os.path.join(ROOT_OUTPUT_DIRECTORY, sanitized_relative_path)
    ...
    for leaf_dir in sorted(leaf_dirs):
        ...
        for scene_prefix in sorted(keyframes_by_scene.keys()):
            for keyframe in sorted(keyframes_by_scene[scene_prefix]):
                ...
                relative_path = os.path.relpath(leaf_dir, ROOT_INPUT_DIRECTORY)
                sanitized_relative_path = sanitize_filename(relative_path)
                output_file = os.path.join(ROOT_OUTPUT_DIRECTORY, sanitized_relative_path, f"{os.path.splitext(keyframe)[0]}_description.txt")
        ...
        logging.info(f"Completed processing film: {leaf_dir}. Time taken: {film_elapsed_time:.2f} seconds")

    genre_end_time = time.time()
    genre_elapsed_time = genre_end_time - genre_start_time
    logging.info(f"Completed processing genre. Time taken: {genre_elapsed_time:.2f} seconds")
...
def main() -> None:
    ...
    logging.info(f"Sentiment analysis for keyframes completed. Total time taken: {overall_elapsed_time:.2f} seconds")
