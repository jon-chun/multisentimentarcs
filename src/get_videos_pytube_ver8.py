...
def download_video(link, download_path, filename_root):
    ...
    try:
        ...
        if file_base_exists(download_path, filename_root):
            message = f"File with base name '{filename_root}' already exists in {download_path}, skipping download."
            log_messages.append(message)
            logging.info(message)
            return "skipped", log_messages, None
...
def download_with_timeout(link, download_path, filename_root, timeout=MAX_DOWNLOAD_TIMESECS):
    ...
    try:
        ...
        message = f"An error occurred while processing video {link}: {e}"
        log_messages.append(message)
        logging.error(message)
