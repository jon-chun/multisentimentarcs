...
def adjust_threshold(current_threshold, scene_count):
    """Adjust the threshold based on the number of scenes detected."""
    if scene_count > (MAX_SCENE_COUNT * (100 / PARTIAL_VIDEO_PERCENTAGE)):
        return current_threshold * THRESHOLD_ADJUSTMENT_UP
    elif scene_count < (MIN_SCENE_COUNT * (100 / PARTIAL_VIDEO_PERCENTAGE)):
...
def process_video_with_retries(input_file_path, output_subdir, video_filename, video_basename, scenes_file_path, current_threshold):
    """Process a video with retries for scene detection."""
    attempt = 0
    ...
    while attempt < MAX_RETRY_ATTEMPTS:
        ...
        logging.info(f"process_video_with_retries(): Partial scene count for {video_filename}: {partial_scene_count}")
        extrapolated_scene_count = (100 / PARTIAL_VIDEO_PERCENTAGE) * partial_scene_count
        logging.info(f"process_video_with_retries(): Extrapolated scene count for {video_filename}: {extrapolated_scene_count}")

        if MIN_SCENE_COUNT <= extrapolated_scene_count <= MAX_SCENE_COUNT:
            logging.info(f"process_video_with_retries(): Partial scene count is within desired range for {video_filename}. Proceeding to full video.")
            valid_threshold_found = True
            break
        else:
            current_threshold_old = current_threshold
            current_threshold = adjust_threshold(current_threshold, partial_scene_count)
            logging.info(f"process_video_with_retries(): Adjusted threshold from {current_threshold_old} to {current_threshold} for {video_filename}")
            logging.warning(f"process_video_with_retries(): Partial scene count ({partial_scene_count}) out of range. Adjusting threshold to {current_threshold} and retrying...")

        attempt += 1

    if valid_threshold_found:
        if full_scene_detection(input_file_path, output_subdir, current_threshold, video_basename, scenes_file_path):
            logging.info(f"process_video_with_retries(): Full scene detection completed for {video_filename} using threshold {current_threshold}.")
        else:
            logging.error(f"process_video_with_retries(): Full scene detection failed for {video_filename} even after finding valid threshold.")
    else:
        logging.error(f"process_video_with_retries(): Failed to find a valid threshold after {MAX_RETRY_ATTEMPTS} attempts for {video_filename}. Skipping...")
...
def partial_scene_detection(input_file_path, output_subdir, threshold, percentage):
    """Detect scenes in a partial video."""
    video_filename = os.path.basename(input_file_path)
    video_filename_no_ext = os.path.splitext(video_filename)[0]
    partial_video_path = os.path.join(output_subdir, f"{video_filename_no_ext}_partial.mp4")
...
def full_scene_detection(input_file_path, output_subdir, threshold, video_basename, scenes_file_path):
    """Perform full scene detection on the video."""
    output_scenes_dir = os.path.join(output_subdir, video_basename)
    os.makedirs(output_scenes_dir, exist_ok=True)
