...
def robust_scaling(series):
    """Apply robust scaling to a series."""
    median = series.median()
    interquartile_range = iqr(series)
    if interquartile_range == 0:
        return series - median
    return (series - median) / interquartile_range
...
def timeseries_norm(df, df_col_list=['vader', 'textblob', 'llama3'], sma_per=10):
    try:
        ...
        for col in df_col_list:
            ...
            scaled_col = robust_scaling(df[col])
            # Compute EMA
            ema_col = scaled_col.ewm(span=int(len(df) * (sma_per / 100)), adjust=False).mean()
            # Normalize using zscore
            normalized_col = zscore(ema_col)
            # Apply smoothing using spline
            spline = UnivariateSpline(df['time_midpoint'], normalized_col, s=1)
