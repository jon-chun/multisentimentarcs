...
def partial_scene_detection(input_file_path, output_subdir, threshold, percentage):
    ...
    print(f"Partial command to execute: {' '.join(command)}")
    
    try:
        subprocess.run(command, check=True)
        logging.info(f"Partial scene detection for {input_file_path} completed. Output written to {output_subdir}")
        print(f"Partial scene detection for {input_file_path} completed. Output written to {output_subdir}")
        ...
        print(f"Error in partial scene detection for {input_file_path}: {e}")
        return 0
...
def process_video_file(input_file_path, output_subdir):
    ...
    for attempt in range(MAX_RETRY_ATTEMPTS):
        ...
        print(f"Partial scene count for {video_filename}: {partial_scene_count}")
        ...
        print(f"Partial scene count ({partial_scene_count}) out of range. Adjusting threshold to {current_threshold} and retrying...")
